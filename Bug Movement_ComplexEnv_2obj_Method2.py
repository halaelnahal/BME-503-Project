...
image = ['Images/7.jpg', 'Images/1.jpg']
ABL=imgparams(image)
...
#        ABL[index].append(1)
        if ABL[index][6]==0 and recentindex!=index:
            ...
            if imgplot!=1:
                imgplot.remove()
#            plt.subplots(1,2,figsize=[100,100])
            plt.subplot(122)
...
def update_plot():
    global imagex, imagey, bug_plot, image_plot1, image_plot2, sr_plot, sl_plot, index, ABL
    bug_plot[0].remove()
#    image_plot[0].remove()
...
#        image_plot=plot(ABL[i][4],ABL[i][5],'b*')
    image_plot1=plot(ABL[0][4],ABL[0][5],'b*',label=ABL[0][2])
    image_plot2=plot(ABL[1][4],ABL[1][5],'k*',label=ABL[1][2])
#    legend1=plt.legend(handles=image_plot1)
#    legend2=plt.legend(handles=image_plot2)
